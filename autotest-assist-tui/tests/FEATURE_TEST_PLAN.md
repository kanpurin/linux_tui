...
| Variable assertion | OK/NG should be based on script variables, not stdout/stderr. | Runtime test for `match_value` with regex-capable exact/contains plus empty/not_empty. |
| Multiple variable checks | A test may need several independent assertions. | Runtime test that all checks must pass. |
| Script `@check` preservation | Script-authored `@check` directives are the source of truth for variable assertions. | Static check that `@check` parsing supports escaped expected values, scans checks without a fixed `MAX_CHECKS` limit, and the old Variable Match menu is not exposed. |
...
| `@tui` output capture | Assertions need clean TUI output, while troubleshooting still needs the raw terminal transcript. | Static check that generated scripts save cleaned `AUTOTEST_TUI_STDOUT`/`AUTOTEST_TUI_TEXT`, raw `AUTOTEST_TUI_TRANSCRIPT`, sent input, `AUTOTEST_TUI_STDERR`, status, and backing files. |
